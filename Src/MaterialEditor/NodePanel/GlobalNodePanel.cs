...
{
    public partial class GlobalNodePanel : NodePanel
    {
        private GlobalNode m_Global;

        public GlobalNodePanel(MainWin main)
            : base(main)
        {
        }
        ...
        public GlobalNode Global
        {
            ...
            set
            {
                m_Global = (GlobalNode)value;

                if (m_Global.RectLabel.X < -999 ||
                    m_Global.RectLabel.Y < -999)
                {
                    m_Global.RectLabel.X = (int)((ClientRectangle.Width / 2 - AutoScrollPosition.X) / GraphScale);
                    m_Global.RectLabel.Y = (int)((ClientRectangle.Height / 2 - AutoScrollPosition.Y) / GraphScale);
                }
            }
        }
        ...
        {
            int centerX = m_Global.RectLabel.X + m_Global.RectLabel.Rect.Width / 2;
            Size totalSize = new Size(0, 0);
            foreach (InputNode input in m_Global.Inputs)
            {
            ...
            int top = m_Global.RectLabel.Y - totalSize.Height;
            foreach (InputNode input in m_Global.Inputs)
            {
            ...
            foreach (GlobalNodeProxy dependent in m_Global.Dependents)
            {
        ...
        protected override Size GetAutoScrollMinSize()
        {
            ...
            if (m_Global != null)
            {
                ...
                size.Width = System.Math.Max(m_Global.RectLabel.Rect.Right, size.Width);
                size.Height = System.Math.Max(m_Global.RectLabel.Rect.Bottom, size.Height);
                foreach (InputNode input in m_Global.Inputs)
                {
        ...
        protected override bool DoMouseDown(MouseEventArgs e)
        {
            if (e.Button == MouseButtons.Left)
            {
                ...
                if (GetScaledRectangle(m_Global.RectLabel.Rect).Contains(point))
                {
                    m_Main.Explorer.SelectNode(m_Global);
                    m_DragBehavior = new GlobalNodeDragBehavior(m_Global);
                    return true;
                }
                ...
                foreach (InputNode input in m_Global.Inputs)
                {
                    ...
                    else if (GetScaledRectangle(input.EllipseLabel.Rect).Contains(point))
                    {
                        ...
                        foreach (GlobalNodeProxy proxy in m_Global.Dependents)
                        {
        ...
        protected override void Draw(Graphics graphics)
        {
            ...
            m_Global.RectLabel.Draw(graphics);
