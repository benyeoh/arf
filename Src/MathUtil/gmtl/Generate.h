 ...
 * -----------------------------------------------------------------
 * File:          Generate.h,v
 * Date modified: 2004/11/16 15:37:12
 * Version:       1.88
 * -----------------------------------------------------------------
 *
 *********************************************************** ggt-head end */
...
#include <MathUtil/gmtl/Util/Assert.h>
#include <MathUtil/gmtl/Util/StaticAssert.h>

#include <MathUtil/gmtl/Vec.h>    // for Vec
#include <MathUtil/gmtl/VecOps.h> // for lengthSquared
#include <MathUtil/gmtl/Quat.h>
#include <MathUtil/gmtl/QuatOps.h>
#include <MathUtil/gmtl/Coord.h>
#include <MathUtil/gmtl/Matrix.h>
#include <MathUtil/gmtl/Util/Meta.h>
#include <MathUtil/gmtl/Math.h>
#include <MathUtil/gmtl/Xforms.h>

#include <MathUtil/gmtl/EulerAngle.h>
#include <MathUtil/gmtl/AxisAngle.h>
...
namespace gmtl
{
   ...
   */
   ...
   inline Matrix<T, 4,4, isAligned>& setFrustum( Matrix<T, 4, 4, isAligned>& result,
...
//      result.mState = Matrix<T, 4, 4>::FULL; // track state
    ...
    */
   template <typename T, bool isAligned >
   inline Matrix<T, 4,4, isAligned>& setOrtho( Matrix<T,4,4, isAligned>& result,
                                          T left, T top, T right,
                                          T bottom, T nr, T fr )
   {
      ...
      result.mData[10] = nf_inv;

      result.mData[12] = -(right + left) * rl_inv;
      result.mData[13] = -(top + bottom) * tb_inv;
      result.mData[14] = (nr) * nf_inv;
      result.mData[15] = T(1);

	 // result.mState = Matrix<T, 4, 4>::FULL;
      return result;
   }
   ...
   template <typename T, bool isAligned>
   inline Matrix<T, 4,4,isAligned>& setOrthoLH( Matrix<T,4,4,isAligned>& result,
                                        T left, T top, T right,
                                        T bottom, T nr, T fr )
   {
	  result.mData[1] = result.mData[2] = result.mData[3] =
    ...
    * @param fovy
    *   The field of view angle, in radians, about the y-axis.
    * @param aspect
    ...
    */
   template <typename T, bool isAligned>
   inline Matrix<T, 4,4, isAligned>& setPerspective( Matrix<T, 4, 4, isAligned>& result,
                                                 T fovy, T aspect, T nr, T fr )
   {
	...
	// Added by Ben
	template<typename T, bool isAligned, bool isAligned1, bool isAligned2, bool isAligned3>
	inline Matrix<T, 4, 4, isAligned>& setView( Matrix<T, 4, 4, isAligned>& result,
									 const Vec<T, 3, isAligned1>& eye,
									 const Vec<T, 3, isAligned2>& at,
									 const Vec<T, 3, isAligned3>& up )
	{
		Vec<T, 3, isAligned1> zAxis = eye - at;
		gmtl::normalize(zAxis);
		
		Vec<T, 3, isAligned3> newUp; 
		if( abs( gmtl::dot(zAxis, up) ) <= 0.98f )
		{
			newUp = up;
		}
		else
		{
			gmtl::cross(newUp, Vec<T, 3, isAligned3>(1.0f, 0.0f, 0.0f), zAxis);
		}
		
		Vec<T, 3, isAligned2> xAxis;
		gmtl::cross(xAxis, newUp, zAxis);
		gmtl::normalize(xAxis);

		Vec<T, 3, isAligned3> yAxis;
		gmtl::cross(yAxis, zAxis, xAxis);

		result[0][0] = xAxis[0];
		result[0][1] = xAxis[1];
		result[0][2] = xAxis[2];
		result[0][3] = -gmtl::dot(xAxis, eye);
		
		result[1][0] = yAxis[0];
		result[1][1] = yAxis[1];
		result[1][2] = yAxis[2];
		result[1][3] = -gmtl::dot(yAxis, eye);

		result[2][0] = zAxis[0];
		result[2][1] = zAxis[1];
		result[2][2] = zAxis[2];
		result[2][3] = -gmtl::dot(zAxis, eye);
	...
	//	result.setState(Matrix<T,4,4>::FULL);
		return result;
	}
	...
	// Added by Ben
	template<typename T, bool isAligned>
	inline Matrix<T, 4, 4, isAligned>& setViewWithAxis( Matrix<T, 4, 4, isAligned>& result,
														const Vec<T, 3, isAligned>& eye, const Vec<T, 3, isAligned>& xAxis, 
														const Vec<T, 3, isAligned>& yAxis, const Vec<T, 3, isAligned>& zAxis )
	{
		result[0][0] = xAxis[0];
		result[0][1] = xAxis[1];
		result[0][2] = xAxis[2];
		result[0][3] = -gmtl::dot(_CAST_VEC3(xAxis), _CAST_VEC3(eye));

		result[1][0] = yAxis[0];
		result[1][1] = yAxis[1];
		result[1][2] = yAxis[2];
		result[1][3] = -gmtl::dot(_CAST_VEC3(yAxis), _CAST_VEC3(eye));

		result[2][0] = zAxis[0];
		result[2][1] = zAxis[1];
		result[2][2] = zAxis[2];
		result[2][3] = -gmtl::dot(_CAST_VEC3(zAxis), _CAST_VEC3(eye));
   ...
   */
    ...
    */
   template< typename DATA_TYPE, unsigned ROWS, unsigned COLS, bool align>
   inline Matrix<DATA_TYPE, ROWS, COLS, align>& setRot( Matrix<DATA_TYPE, ROWS, COLS, align>& result, const AxisAngle<DATA_TYPE>& axisAngle )
   {
      /* @todo set this a compile time assert... */
      gmtlASSERT( ROWS >= 3 && COLS >= 3 && ROWS <= 4 && COLS <= 4 &&
