...
template <typename DATA_TYPE, unsigned ROWS, unsigned COLS, bool isAligned=false> 
class Matrix
{
public:
...
//#endif
   ...
   */
   ...
   public:
      ...
      RowAccessor(Matrix<DATA_TYPE,ROWS,COLS, isAligned>* mat, const unsigned row)
         : mMat(mat), mRow(row)
      {
         gmtlASSERT(row < ROWS);
         gmtlASSERT(NULL != mat);
      ...
      Matrix<DATA_TYPE,ROWS,COLS,isAligned>*  mMat;
   ...
   */
   ...
   public:
      ...
      ConstRowAccessor( const Matrix<DATA_TYPE,ROWS,COLS, isAligned>* mat,
                        const unsigned row )
         : mMat( mat ), mRow( row )
      {
         gmtlASSERT( row < ROWS );
      ...
      const Matrix<DATA_TYPE,ROWS,COLS,isAligned>*  mMat;
      unsigned                      mRow;    /** The row being accessed */
   };
...
class Matrix<DATA_TYPE, ROWS, COLS, true>
{
public:
	// This is a hack to work around a bug with GCC 3.3 on Mac OS X where
	// boost::is_polymorphic returns a false positive.  The details can be
	...
	*/
	...
	public:
		...
		RowAccessor(Matrix<DATA_TYPE,ROWS,COLS, true>* mat, const unsigned row)
			: mMat(mat), mRow(row)
		{
			gmtlASSERT(row < ROWS);
			gmtlASSERT(NULL != mat);
		...
		Matrix<DATA_TYPE,ROWS,COLS,true>*  mMat;
		unsigned                      mRow;    /** The row being accessed */
	};
	...
	*/
	...
	public:
		...
		ConstRowAccessor( const Matrix<DATA_TYPE,ROWS,COLS, true>* mat,
			const unsigned row )
			: mMat( mat ), mRow( row )
		{
			gmtlASSERT( row < ROWS );
		...
		const Matrix<DATA_TYPE,ROWS,COLS,true>*  mMat;
		unsigned                      mRow;    /** The row being accessed */
	};
...
public:
	...
	*  to get the correct result at the cost of some performance.
	*/
	_ALIGN(16) DATA_TYPE mData[COLS*ROWS];
};
...
typedef Matrix<float, 4, 4, true> MatrixA44f;
typedef Matrix<float, 4, 4, true> MatrixA33f;

#define _CAST_MAT44(mat) *((gmtl::Matrix44f*)&mat)
#define _CAST_MATA44(mat) *((gmtl::MatrixA44f*)&mat)
#define _CAST_MAT33(mat) *((gmtl::Matrix33f*)&mat)
#define _CAST_MATA33(mat) *((gmtl::MatrixA33f*)&mat)
