...
#include <MathUtil/gmtl/Helpers.h>
 ...
 */
#ifndef GMTL_NO_METAPROG
template<class DATA_TYPE, unsigned SIZE, typename REP=meta::DefaultVecTag, bool isAligned=false>
class VecBase
{
protected:
...
class VecBase
#else
class VecBase<DATA_TYPE,SIZE,meta::DefaultVecTag,false>
#endif
{
public:
...
#ifndef GMTL_NO_METAPROG
   template<typename REP2, bool align>
   VecBase(const VecBase<DATA_TYPE, SIZE, REP2,align>& rVec)
   {
#ifdef GMTL_COUNT_CONSTRUCT_CALLS
      gmtl::helpers::VecCtrCounterInstance()->inc();
#endif
...
#ifdef GMTL_NO_METAPROG
   template<bool align>
   inline VecType& operator=(const VecBase<DATA_TYPE,SIZE,align>& rhs)
   {
      for(unsigned i=0;i<SIZE;++i)
      {
         mData[i] = rhs[i];
...
#else
   template<typename REP2, bool align>
   inline VecType& operator=(const VecBase<DATA_TYPE,SIZE,REP2,align>& rhs)
   {
      for(unsigned i=0;i<SIZE;++i)
      {
         mData[i] = rhs[i];
...
class VecBase
#else
class VecBase<DATA_TYPE,SIZE,meta::DefaultVecTag,true>
#endif
{
public:
	/// The datatype used for the components of this VecBase.
	...
	typedef VecBase<DATA_TYPE, SIZE> VecType;
#else
	typedef VecBase<DATA_TYPE, SIZE, meta::DefaultVecTag, true> VecType;
#endif
...
public:
	...
	* @param rVec    the VecBase object to copy
	*/
	VecBase(const VecBase<DATA_TYPE, SIZE,meta::DefaultVecTag,true>& rVec)
	{
#ifdef GMTL_COUNT_CONSTRUCT_CALLS
		gmtl::helpers::VecCtrCounterInstance()->inc();
#endif
		...
		for(unsigned i=0;i<SIZE;++i)
			mData[i] = rVec.mData[i];
#else
		gmtl::meta::AssignVecUnrolled<SIZE-1, VecBase<DATA_TYPE,SIZE,meta::DefaultVecTag,true> >::func(*this, rVec);
#endif
	}
...
public:
	/// The array of components.
	_ALIGN(16) DATA_TYPE mData[SIZE];
};
