...
CRTextureCube* CRResourceMgrD3D11::DoCreateTextureCube(uint size, uint mipMapLevel, eRTexFormat format, eRTexUsage usage)
{
	...
	ID3D11Texture2D* pTex2D = CreateD3DTexture(width, height, mipMapLevel, 6, TexFormatToD3D11Full(format), TexUsageToD3D11(usage), D3D11_BIND_SHADER_RESOURCE, 0);
...
CRTexture2D* CRResourceMgrD3D11::DoCreateTexture2D(uint width, uint height, uint mipMapLevel, eRTexFormat format, eRTexUsage usage)
{
	...
	ID3D11Texture2D* pTex2D = CreateD3DTexture(width, height, mipMapLevel, 1, TexFormatToD3D11Full(format), TexUsageToD3D11(usage), D3D11_BIND_SHADER_RESOURCE, 0);
	pD3DTex->SetD3DTexture(pTex2D, NULL);
	pTex2D->Release();
