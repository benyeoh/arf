...
void ProcessInput()
{
	...
	_LOOPi(numActiveButtons)
	{
		...
		if(pActiveButtons[i] == IMC_RIGHTBUTTON)
		{
			...
			{			
				...
				gmtl::MatrixA33f rotToWorldSpaceMat;
				...
				rotAxis.setAngle(length * g_TimeDT * 10.0f);

				// Transform eye direction
				gmtl::MatrixA33f cameraRotMat;
	...
	_LOOPi(numActiveKeys)
	{
		...
		case IKC_W:
			{
				_CAST_VEC3(g_EyePos) = _CAST_VEC3(g_EyePos) + g_TimeDT * _CAST_VEC3(g_EyeDir) * 100.0f;
				break;
			}
		...
		case IKC_A:
			{
				...
				_CAST_VEC3(g_EyePos) = _CAST_VEC3(g_EyePos) - g_TimeDT * _CAST_VEC3(right) * 100.0f;
				break;
			}
