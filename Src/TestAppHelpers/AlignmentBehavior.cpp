...
void 
AlignmentBehavior::Update(float timeDT, AHComponent* pCurFlocking, AHComponent** ppOthers, uint numOthers, gmtl::VecA3f& newDir)
{
	...
	_LOOPi(numOthers)
	{
		...
		if(pOtherEntity != pCurEntity)
		{
			...
			float length;
			VecLength(&length, &dirToOther);
			
			if(length < m_Radius)
				VecVecAdd(&tempDir, &tempDir, &(pOtherFlocking->GetDir()));
		}
	}
