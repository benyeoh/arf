...
void 
CohesionBehavior::Update(float timeDT, AHComponent* pCurFlocking, AHComponent** ppOthers, uint numOthers, gmtl::VecA3f& newDir)
{
	...
	_LOOPi(numOthers)
	{
		...
		if(pOtherEntity != pCurEntity)
		{
			...
			if(length < m_Radius)
			{
				VecVecAdd(&tempDir, &tempDir, &dirToOther);
			}
		}
	}
