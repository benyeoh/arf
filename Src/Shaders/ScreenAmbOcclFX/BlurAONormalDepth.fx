...
#define DoInit \
	...
	float occl			= center.x * g_FilterWeights[0]; \
	float3 worldPos		= GetWorldPosFromVert(input.farPlaneOffset0.xyz, curDepth); \
	float weightTotal	= g_FilterWeights[0]; \
	float3 normal		= normalize( tex2D(g_SampNormal, input.texUVTap0.xy).xyz * 2.0f - 1.0f); \
...
#define DoBlurSample( i, inUV, offset ) \
	...
	float3 curPos##i		= GetWorldPosFromVert(offset, UncompressDepth(tap##i.yzw)); \
	float3 tap##i##Diff		= curPos##i - worldPos; \
	float weightScale##i	= FindProjection( tap##i##Diff, normal ); \
	curWeight				= g_FilterWeights[i] * weightScale##i; \
	occl					+= (tap##i##.x * curWeight); \
	weightTotal				+= curWeight;
	

/*	
	float4 tap##i			= tex2D(g_SampAO, input.texUVTap##i##); \
	float tap##i##Diff		= UncompressDepth(tap##i##.yzw) - curDepth ; \
	float weightScale##i	= 1.0f - saturate( (tap##i##Diff * tap##i##Diff) / (0.0001f * 0.0001f) ); \
	curWeight				= g_FilterWeights[i] * weightScale##i; \
	occl					+= (tap##i##.x * curWeight); \
	weightTotal				+= curWeight;
*/	
...
BlurAODepth7SamplesPS(VS_OUT_BlurAODepth input)
{
	...
	output.color	= float4(occl / weightTotal, center.y, center.z, center.w);
